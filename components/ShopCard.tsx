...
export default function ShopCard({ shop, visited }: Props) {
  const saved = useSyncExternalStore(
    subscribeToSaved,
    () => getSavedSlugs().includes(shop.slug),
    () => false
  )
  ...
  return (
    <div
      className={`group relative bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-[#E0CEBC] ${
        visited ? "ring-2 ring-[#E85D75]" : ""
      }`}
    >
      {/* Save button — sibling to Link, not nested inside it */}
      <button
        onClick={handleSave}
      ...
      >
        <Heart
          className={`w-4 h-4 transition-colors ${
            saved ? "fill-[#E85D75] text-[#E85D75]" : "text-[#8B5E3C]"
          }`}
        />
      </button>
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#8B5E3C] mt-0.5">Signature: {shop.name}</p>
            </div>
          ...
          <p className="text-sm text-[#5C3317] line-clamp-2 leading-relaxed">{shop.description}</p>

          <div className="mt-3 flex flex-wrap gap-1">
            {shop.category.map((cat) => (
              <span key={cat} className="text-xs bg-[#F2E8DC] text-[#5C3317] px-2 py-0.5 rounded-full">
                {cat}
              </span>
            ))}
